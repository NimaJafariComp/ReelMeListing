...
class HttpComfyUiClient:
    ...
    def submit_and_wait(self, workflow: dict[str, object]) -> Path:
        ...
        for _ in range(600):
            ...
            if completed:
                outputs = completed["outputs"]["16"]["images"]
                image = outputs[0]
...
def _bridges() -> list[LtxBridgeCandidate]:
    return [
        LtxBridgeCandidate(
            ...
            prompt=(
                "Use only visibly overlapping front geometry; freeze structure and use minimal "
                "lateral continuation."
            ),
            reason=(
                "Candidate only: reject if roof, windows, driveway, trees, or wall geometry "
                "bends or shifts."
            ),
        ),
        LtxBridgeCandidate(
            ...
            duration_seconds=0.75,
            prompt=(
                "Use only if patio and backyard overlap is visibly sufficient; otherwise use a "
                "clean cut."
            ),
            reason=(
                "Optional candidate only: reject unless overlap is obvious and all geometry "
                "remains fixed."
            ),
        ),
        LtxBridgeCandidate(
            ...
            prompt=(
                "Align the two front views, freeze camera and geometry, and change only "
                "daylight, sky color, and practical/interior lighting."
            ),
            reason="Candidate only: reject if the house, driveway, trees, windows, or patio shift.",
        ),
    ]
...
def evaluate_ltx_render(manifest_path: Path, output_dir: Path) -> LtxQualityReport:
    ...
    for clip in manifest.clips:
        capture = cv2.VideoCapture(clip.generated_path)
        ...
        capture.release()
        if not frames:
            raise ValueError(f"No decodable frames in {clip.generated_path}")
        ...
        metrics = TemporalMetrics(
            ...
            duration_seconds=clip.video.duration_seconds,
    ...
    with worksheet.open("w", encoding="utf-8", newline="") as file:
        ...
        for bridge in manifest.bridge_candidates:
            writer.writerow(
                {
                    "kind": "bridge",
                    "name": bridge.candidate_id,
                    "path_or_candidate": f"{bridge.from_view} -> {bridge.to_view}",
                    "decision": "",
                    "reviewer": "",
                    "notes": bridge.reason,
                }
    ...
    overall = (
        VideoDecision.REJECTED
        if any(item.decision is VideoDecision.REJECTED for item in reports)
        else VideoDecision.QUEUED_FOR_HUMAN_REVIEW
    )
