...
function App() {
  ...
  const [sameCompositionConfirmed, setSameCompositionConfirmed] = useState(false);
  ...
  function addBridge() {
    if (bridgeFrom === bridgeTo || bridgePairs.has(`${bridgeFrom}-${bridgeTo}`) || (bridgeKind === "lighting" && !sameCompositionConfirmed)) return;
    setBridges((current) => [...current, { from_index: bridgeFrom, to_index: bridgeTo, kind: bridgeKind, duration_seconds: bridgeDuration }]);
    setSameCompositionConfirmed(false);
  }
  ...
  return <main>
    ...
    {tab === "studio" && <section className="panel studio" id="studio"><div className="panel-head"><div><p className="kicker">01 / PROPERTY STORYBOARD</p><h2>Upload and label every view.</h2></div><label className="upload">Add photos<input type="file" accept="image/*" multiple onChange={(event) => addPhotos(event.target.files)} /></label></div>
      ...
      {photos.length > 1 && <section className="bridge-workflow"><aside className="bridge-guide"><p className="kicker">OPTIONAL · LTX CAMERA BRIDGE</p><h3>Connect compatible views.</h3><p>Use an invented bridge only when the camera could plausibly move from one view to the next.</p><ul><li><b>Camera move:</b> same or adjacent area, different angle.</li><li><b>Lighting only:</b> confirm the pair has essentially identical framing and a different time of day.</li><li><b>Use a cut:</b> unrelated areas or uncertain geometry.</li></ul><small>Synthetic visualization—not a verified walkthrough.</small></aside><div className="bridge-setup"><div className="bridge-setup-head"><div><b>Create a bridge</b><span>It will be generated and checked by LTX QA.</span></div><span>{bridges.length} planned</span></div><div className="bridge-form"><label>Start view<select value={bridgeFrom} onChange={(event) => { setBridgeFrom(+event.target.value); setSameCompositionConfirmed(false); }}>{photos.map((_, index) => <option key={index} value={index}>{photoLabel(index)}</option>)}</select></label><label>End view<select value={bridgeTo} onChange={(event) => { setBridgeTo(+event.target.value); setSameCompositionConfirmed(false); }}>{photos.map((_, index) => <option key={index} value={index}>{photoLabel(index)}</option>)}</select></label><fieldset><legend>Transition intent</legend><label><input type="radio" checked={bridgeKind === "spatial"} onChange={() => { setBridgeKind("spatial"); setSameCompositionConfirmed(false); }} /> Camera move</label><label><input type="radio" checked={bridgeKind === "lighting"} onChange={() => setBridgeKind("lighting")} /> Lighting only</label></fieldset>{bridgeKind === "lighting" && <label className="composition-check"><input type="checkbox" checked={sameCompositionConfirmed} onChange={(event) => setSameCompositionConfirmed(event.target.checked)} /> I confirm these photos use essentially the same camera position and framing.</label>}<label className="bridge-length">Length <output>{bridgeDuration.toFixed(1)}s</output><input type="range" min="0.75" max="5" step="0.25" value={bridgeDuration} onChange={(event) => setBridgeDuration(+event.target.value)} /></label><button disabled={bridgeFrom === bridgeTo || bridgePairs.has(`${bridgeFrom}-${bridgeTo}`) || (bridgeKind === "lighting" && !sameCompositionConfirmed)} onClick={addBridge}>Plan bridge</button></div>{bridges.length > 0 && <div className="bridge-list">{bridges.map((bridge, index) => <article key={`${bridge.from_index}-${bridge.to_index}`}><span>{String(index + 1).padStart(2, "0")}</span><div><b>{photoLabel(bridge.from_index)} → {photoLabel(bridge.to_index)}</b><p>{bridge.kind === "spatial" ? "Invented camera move" : "Lighting-only transition"} · {bridge.duration_seconds.toFixed(1)} seconds</p></div><button onClick={() => setBridges((current) => current.filter((item) => item !== bridge))}>Remove</button></article>)}</div>}</div></section>}
      ...
      <button className="primary" disabled={photos.length < 2} onClick={generateReel}>Generate reel <span>→</span></button><p className="reel-timing-note">* <b>Bridge length</b> is the duration of an invented LTX camera move between a selected compatible pair. <b>Scene dissolve</b> is the crossfade duration used to join independent shots where you choose a deliberate edit instead of an invented bridge.</p></section>}
    ...
    <footer><span>{status}</span><span>Runs locally · generated bridges are labeled</span></footer>
  </main>;
}
