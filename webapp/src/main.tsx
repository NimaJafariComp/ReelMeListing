...
function App() {
  ...
  async function editPhoto() {
    if (!photos[0]) return; setEditing(true); setStatus("Uploading and checking the source photo…");
    try {
      const [source_path] = await upload([photos[0].file]);
  ...
  return <main>
    ...
    {tab === "edit" && <section className="panel editor"><p className="kicker">02 / IMAGE EDITING</p><h2>Set the daylight-to-dusk treatment.</h2><p>The InstructPix2Pix editor uses the selected local runtime. A photo must pass input QA before it can generate candidates.</p><textarea value={editPrompt} onChange={(event) => setEditPrompt(event.target.value)} /><div className="edit-actions"><span>{photos[0] ? `Source: ${photos[0].file.name}` : "Add photos in Reel studio first."}</span><button className="primary" disabled={!photos[0] || editing} onClick={editPhoto}>{editing ? "Generating…" : "Generate edit candidates"} <span>→</span></button></div></section>}
